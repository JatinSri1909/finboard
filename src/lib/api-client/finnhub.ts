...
export class FinnhubClient {
  ...
  async getCompanyProfile(symbol: string): Promise<ApiResponse> {
    const url = `${this.baseUrl}/stock/profile2?symbol=${symbol}&token=${this.apiKey}`;
    return this.makeRequest(url);
  }

  async getCandles(symbol: string, resolution: string = 'D', from?: number, to?: number): Promise<ApiResponse> {
    const fromTimestamp = from || Math.floor(Date.now() / 1000) - 365 * 24 * 60 * 60; // 1 year ago
  ...
  async getMarketStatus(): Promise<ApiResponse> {
    const url = `${this.baseUrl}/stock/market-status?token=${this.apiKey}`;
    return this.makeRequest(url);
  }
